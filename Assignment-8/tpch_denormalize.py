...
def log_map(line):
    yield ({'orderkey':line[0], 'clerk':line[1], 'comment': line[2], 'custkey': line[3], 'order_priority': line[4], 'orderdate': line[5], 'orderstatus' : line[6],'part_names':line[9], 'ship_priority':line[7], 'totalprice': line[8]})
...
def rows_to_list(key_vals, key_col, list_col):
    ...
    return spark.createDataFrame(key_list_rdd, schema=[key_col, list_col])

def main(input_keyspace,output_keyspace):
    orders = df_table(input_keyspace,'orders').cache()
    line_item = df_table(input_keyspace,'lineitem').cache()
    parts = df_table(input_keyspace,'part').cache()
    parts_name = spark.sql("SELECT o.orderkey,o.totalprice,p.name FROM orders o JOIN lineitem l ON (o.orderkey = l.orderkey)  JOIN part p ON (p.partkey = l.partkey)")
    parts_name = parts_name.groupBy(parts_name['o.orderkey'],parts_name['o.totalprice']).agg(functions.collect_set(parts_name['p.name']))
    parts_name = parts_name.rdd.map(tuple)
    order_name_temp = spark.createDataFrame(parts_name, schema=['orderkey', 'total_price','part_names'])
    order_name_temp.registerTempTable('order_part')
    final = spark.sql("SELECT a.*, b.part_names FROM orders a JOIN order_part b ON (a.orderkey = b.orderkey)")
    final.show(5)
