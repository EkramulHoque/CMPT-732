...
def main(input_keyspace,output_keyspace):
    ...
    parts_name = spark.sql("SELECT o.orderkey,p.name FROM orders o JOIN lineitem l ON (o.orderkey = l.orderkey)  JOIN part p ON (p.partkey = l.partkey)")
    order_name_temp = rows_to_list(parts_name, 'orderkey', 'part_names')
