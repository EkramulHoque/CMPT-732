...
def main(key_space,out_dir,order_keys):
	...
	orders = read_table(key_space,'orders').where('orderkey in' + keys)
	...
	result = result.groupBy(result['o.orderkey'],result['o.totalprice']).agg(functions.collect_set(result['p.name']))
