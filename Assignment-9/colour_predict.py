...
def main(inputs):
    ...
    pipelines = [
        ('RGB-MLP',Pipeline(stages=[rgb_assembler,word_indexer,classifier_mlp])),
        ('RGB-Forest', Pipeline(stages=[rgb_assembler,word_indexer,classifier_fr])),
        ('LAB-MLP', Pipeline(stages=[sqlTrans,lab_assembler,word_indexer,classifier_mlp])),
        ('LAB-Forest', Pipeline(stages=[sqlTrans,lab_assembler,word_indexer,classifier_fr]))
                     ]
