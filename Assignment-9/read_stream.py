...
def main(topic):
    ...
    req_df = spark.sql("""Select SUM(x) as sumx, SUM(y) as sumy, SUM(xy) as sumxy, SUM(x2) as sumx2, COUNT(*) as n from updated_tbl""")
    slope = (req_df['sumxy']-((req_df['sumx']*req_df['sumy'])/req_df['n']))/(req_df['sumx2']-(req_df['sumx']*req_df['sumx'])/req_df['n'])
    ...
    stream = req_df.writeStream.format('console').outputMode('update').start()
